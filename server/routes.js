...
module.exports = function (app) {
  ...
  router.post("/elastic-search/searchData", (req, res) => {
    ...
    searchModule.search2(searchKey, function (data) {
      for (i = 0; i < data.length; i++) {
        data[i]._source.content =
          data[i]._source.content.substring(0, 350) + "...";
      }
      return res.json({ success: true, data: data });
    });
  });
  ...
  router.post("/putCaseReport2", (req, res) => {
    ...
    var nID2index = new Map();
    var k = 0;
    for (k = 0; k < caseReport.entities.length; k++) {
      // map nodes to text from entities
    ...
    for (k = 0; k < caseReport.triggers.length; k++) {
      // map nodes to text from triggers
    ...
    for (k = 0; k < caseReport.relations.length; k++) {
      ...
      const nodeID_1 = eID2nID.has(eventID_1)
        ? eID2nID.get(eventID_1)
        : eventID_1;
      const nodeID_2 = eID2nID.has(eventID_2)
      ...
      caseReport.graph_index_data_before.push([
        n1start,
        n1end,
        nodeID_1,
        n2start,
  ...
  router.post("/deletePendingCaseReport", (req, res) => {
