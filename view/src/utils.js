...
export const buildGraphFromGraphData = graphData => {
  ...
  for (i = 0; i < graphData.relations.length; i++) {
    ...
    const sourceID = eID2nID.has(eventID_1)
      ? eID2nID.get(eventID_1)
      : eventID_1;
    const targetID = eID2nID.has(eventID_2)
      ? eID2nID.get(eventID_2)
      : eventID_2;

    var nodeText;
    if (!nodeSet.has(sourceID)) {
      nodeText = graphData.text.substring(
        nID2index.get(sourceID)[0],
        nID2index.get(sourceID)[1]
      );
    ...
    if (!nodeSet.has(targetID)) {
      nodeText = graphData.text.substring(
        nID2index.get(targetID)[0],
        nID2index.get(targetID)[1]
      );
...
const addOverlapToElements = (
  ...
  if (!nodeSet.has(childID)) {
    // console.log(targetID);
    nodeText = graphData.text.substring(
      nID2index.get(childID)[0],
      nID2index.get(childID)[1]
    );
    // nodeText = nodeText.length>20 ? nodeText.substring(0,20) : nodeText;
    elements.push({
      data: {
...
export const buildGraphElementsFromGraphData = graphData => {
  ...
  for (i = 0; i < graphData.equivs.length; i++) {
    ...
    for (k = 2; k < graphData.equivs[i].length; k++) {
      ...
      const parentID = eID2nID.has(parentNode)
        ? eID2nID.get(parentNode)
        : parentNode;
      const childID = eID2nID.has(childNode)
        ? eID2nID.get(childNode)
        : childNode;
      addOverlapToElements(
        graphData,
        elements,
        parentID,
        childID,
...
export const buildSubGraphElementsFromGraphData = (graphData, queryNodes) => {
  ...
  for (i = 0; i < graphData.equivs.length; i++) {
    ...
    for (k = 2; k < graphData.equivs[i].length; k++) {
      ...
      if (
        queryNodesSet2ndLayer.has(parentID) ||
        queryNodesSet2ndLayer.has(childID)
      ) {
        addOverlapToElements(
          graphData,
          elements,
  ...
  for (i = 0; i < graphData.relations.length; i++) {
    ...
    // }
    if (
      queryNodesSet2ndLayer.has(sourceID) ||
      queryNodesSet2ndLayer.has(targetID)
    ) {
